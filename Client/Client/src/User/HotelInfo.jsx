...
import { FaStar, FaMapMarkerAlt, FaBed, FaUsers, FaRupeeSign, FaBuilding } from "react-icons/fa";
  ...
  return (
    <>
      ...
      <div className="relative h-96 w-full">
        <img
          src={(hotel.images && hotel.images[0]) || fallbackImages[0]}
          alt="Hotel"
          className="w-full h-full object-cover brightness-90"
        />
        <div className="absolute inset-0 bg-gradient-to-b from-black/40 to-black/20 flex flex-col justify-end p-6">
          <h1 className="text-4xl font-bold text-white">{hotel.name || "Hotel Name"}</h1>
          <div className="flex items-center gap-2 mt-2 text-yellow-400">
            <FaStar />
            <span className="font-semibold">{hotel.rating || 0}</span>
            <span className="text-white/70 text-sm">({hotel.reviewsCount || 0} reviews)</span>
          </div>
              ...
              <div className="mt-4 flex items-start gap-2 text-gray-600">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    {hotel.location.distanceFromAirport} from airport, {hotel.location.distanceFromRailwayStation} from railway station
                  </p>
                </div>
              </div>
            )}
            ...
            hotel.rooms.map((room) => (
              <div key={room.roomId} className="bg-white p-6 rounded-2xl shadow space-y-4">
                ...
                <div className="flex gap-2 overflow-x-auto">
                  {(room.images || fallbackImages).slice(0, 5).map((img, index) => (
                    <img
                      key={index}
                      src={img}
                      alt={`Room ${index}`}
        ...
        <div className="lg:col-span-1 sticky top-20 space-y-4">
          <div className="bg-white p-6 rounded-2xl shadow space-y-4">
            ...
            <p className="text-gray-700">
              Starting from <span className="font-bold text-green-600">₹{hotel.rooms?.[0]?.price?.toLocaleString() || 0}</span> / night
            </p>
